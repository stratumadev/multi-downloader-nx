...
export default class Crunchy implements ServiceClass {
    ...
    public async cli() {
        ...
        if (argv.dlFonts) {
            ...
            const selected = await this.downloadFromSeriesID(argv.series, {
                ...argv
            });
            if (selected.isOk) {
            ...
            for (const select of selected as Partial<CrunchyEpMeta>[]) {
                if (
                    !(await this.downloadEpisode(select as CrunchyEpMeta, {
                        ...argv,
                        skipsubs: false
                    }))
                ) {
    ...
    private async productionToken() {
        ...
        if (!tokenReq.ok || !tokenReq.res) {
            ...
            return {
                isOk: false,
                reason: new Error('Failed to get Production Token')
            };
        }
        ...
        if (!tokens) {
            ...
            return {
                isOk: false,
                reason: new Error('Failed to find Production Token in js')
            };
        }
    ...
    public async doAuth(data: AuthData): Promise<AuthResponse> {
        ...
        const authReqOpts: reqModule.Params = {
            ...
            headers: {
                ...
                Authorization: `Basic ${await this.productionToken()}`
            },
            body: authData
        };
        ...
        return;*/
            ...
            const authReqOpts: reqModule.Params = {
                ...
                headers: {
                    ...api.crunchyAuthHeader,
                    Authorization: `Basic ${await this.productionToken()}`,
                    Cookie: `etp_rt=${this.token.refresh_token}`
                },
                body: authData
            };
            const authReq = await this.req.getData(api.beta_auth, authReqOpts);
    ...
    public async doSearch(data: SearchData): Promise<SearchResponse> {
        ...
        if (!searchReq.ok || !searchReq.res) {
            ...
            return {
                isOk: false,
                reason: new Error('Search failed. No more information provided')
            };
        }
    ...
    public async getSeasonById(id: string, numbers: number, e: string | undefined, but: boolean, all: boolean): Promise<ResponseBase<CrunchyEpMeta[]>> {
        ...
        if (!showInfoReq.ok || !showInfoReq.res) {
            ...
            return {
                isOk: false,
                reason: new Error('Show request failed. No more information provided.')
            };
        }
        ...
        if (this.api == 'android') {
            ...
            if (!reqEpsList.ok || !reqEpsList.res) {
                ...
                return {
                    isOk: false,
                    reason: new Error('Episode List request failed. No more information provided.')
                };
            }
            //CrunchyEpisodeList
    ...
    public async getObjectById(e?: string, earlyReturn?: boolean, external_id?: boolean): Promise<ObjectInfo | Partial<CrunchyEpMeta>[] | undefined> {
        ...
        if (this.api == 'android') {
            ...
            const objectReq = await this.req.getData(
                `${api.cms}/objects/${doEpsFilter.values.join(',')}?force_locale=&preferred_audio_language=ja-JP&locale=${this.locale}`,
                AuthHeaders
            );
            if (!objectReq.ok || !objectReq.res) {
                console.error('Objects Request FAILED!');
    ...
    > {
        ...
        for (const mMeta of medias.data) {
            ...
            if (options.chapters) {
                ...
                if (!chapterRequest.ok || !chapterRequest.res) {
                    ...
                    if (!oldChapterRequest.ok || !oldChapterRequest.res) {
                        ...
                        compiledChapters.push(`CHAPTER${compiledChapters.length / 2 + 1}=${startFormatted}`, `CHAPTER${compiledChapters.length / 2 + 1}NAME=Opening`);
            ...
            let curStream: undefined | (typeof streams)[0] = undefined;
            if (!dlFailed) {
            ...
            if (options.novids && options.noaudio) {
                if (playStream) {
                    await this.refreshToken(true, true);
                    await this.req.getData(`https://cr-play-service.prd.crunchyrollsvc.com/v1/token/${currentVersion ? currentVersion.guid : currentMediaId}/${playStream.token}`, {
                        ...{ method: 'DELETE' },
                        ...AuthHeaders
                    });
                }
                    ...
                    if (streamPlaylistBody.match('MPD')) {
                        ...
                        if (playStream) {
                            ...
                            await this.req.getData(
                                `https://cr-play-service.prd.crunchyrollsvc.com/v1/token/${currentVersion ? currentVersion.guid : currentMediaId}/${playStream.token}`,
                                { ...{ method: 'DELETE' }, ...AuthHeaders }
                            );
                        }

                        //Parse MPD Playlists
                        const streamPlaylists = await parse(
                            streamPlaylistBody,
                            langsData.findLang(langsData.fixLanguageTag(pbData.meta.audio_locale as string) || ''),
                            curStream.url.match(/.*\.urlset\//)[0]
                        );
                        ...
                        if (
                            ...
                            const authData = (await decReq.res.json()) as {
                                custom_data: string;
                                token: string;
                            };
                ...
                keys[key.kid] = key.key;
                            ...
                            if (this.cfg.bin.mp4decrypt || this.cfg.bin.shaka) {
                                ...
                                if (audioDownloaded) {
                                    ...
                                    const decryptAudio = exec(
                                        this.cfg.bin.shaka ? 'shaka' : 'mp4decrypt',
                                        this.cfg.bin.shaka ? `"${this.cfg.bin.shaka}"` : `"${this.cfg.bin.mp4decrypt}"`,
                                        commandAudio
                                    );
