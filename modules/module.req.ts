...
class Req {
    ...
    async getData<T = string>(durl: string, params?: Params) {
        ...
        try {
            ...
            if (error.res && error.res.body && error.response.statusCode && error.response.statusCode != 404 && error.response.statusCode != 403) {
    ...
    setNewCookie(setCookie: Record<string, string>, isAuth: boolean, fileData?: string) {
        ...
        for (const uCookie of usefulCookies.sess) {
            ...
            if (isAuth || (this.nosess && parsedCookie[uCookie]) || (parsedCookie[uCookie] && !this.checkSessId(this.session[uCookie]))) {
    ...
    checkCookieVal(chcookie: Record<string, string>) {
        return chcookie && chcookie.toString() == '[object Object]' && typeof chcookie.value == 'string' ? true : false;
    }
...
export { buildProxy, usefulCookies, Req };
