...
class Merger {
    ...
    public static checkMerger(
        bin: {
            mkvmerge?: string;
            ffmpeg?: string;
        },
        useMP4format: boolean,
        force: AvailableMuxer | undefined
    ): {
        MKVmerge?: string;
        FFmpeg?: string;
    } {
    ...
    public static makeFontsList(
        ...
        let fontsNameList: Font[] = [];
        const fontsList: { name: string; path: string; mime: string }[] = [],
            subsList: string[] = [];
        let isNstr = true;
        for (const s of subs) {
